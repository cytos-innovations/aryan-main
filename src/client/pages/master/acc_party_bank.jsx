...
export default function AccPartyBank() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [...QK, qs],
    queryFn: () => invoke("get_party_banks", { qs }),
  });
  ...
  const createMut = useMutation({
    mutationFn: (f) =>
      invoke("create_party_bank", {
        name: f.name,
        code: f.code ? Number(f.code) : null,
        location: f.location || null,
      }),
    onSuccess: () => { toast.success("Party bank created"); inv(); closeDialog(); },
    onError: (e) => toast.error(String(e)),
  });

  const updateMut = useMutation({
    mutationFn: (f) =>
      invoke("update_party_bank", {
        id: f.id,
        code: Number(f.code),
        name: f.name,
        location: f.location || null,
      }),
    onSuccess: () => { toast.success("Party bank updated"); inv(); closeDialog(); },
    onError: (e) => toast.error(String(e)),
  });

  const toggleMut = useMutation({
    mutationFn: ({ id, is_active }) =>
      invoke("toggle_party_bank_active", { id, isActive: is_active }),
    onSuccess: () => inv(),
    onError: (e) => toast.error(String(e)),
  });

  const deleteMut = useMutation({
    mutationFn: (id) => invoke("delete_party_bank", { id }),
    onSuccess: () => { toast.success("Party bank deleted"); inv(); setDeleteTarget(null); },
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (isEditMode && !form.code) { toast.error("Bank code is required"); return; }
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Active",
      cell: ({ row }) => (
        <Can perm="acc-party-bank:update">
          <Switch
      ...
      cell: ({ row }) => (
        <div className="flex gap-1">
          <TooltipProvider>
            ...
            <Can perm="acc-party-bank:delete">
              <Tooltip>
                <TooltipTrigger asChild>
    ...
    <div className="p-6 space-y-4">
      <Card>
        <CardHeader className="pb-3">
          <CardTitle>Party Bank</CardTitle>
        </CardHeader>
        <CardContent>
          <DataTable
            columns={columns}
      ...
      <Dialog open={dialog.open} onOpenChange={(o) => !o && closeDialog()}>
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleSubmit} onKeyDown={enterNav} className="space-y-4">
            ...
            <Field>
              <FieldLabel>Location / Branch</FieldLabel>
              <Input
                value={form.location}
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={isPending}>
                {isPending ? "Saving…" : isEditMode ? "Save Changes" : "Create Bank"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Party Bank</AlertDialogTitle>
            <AlertDialogDescription>
