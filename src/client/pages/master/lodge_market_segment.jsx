...
const QK = ["market-segments"];
const EMPTY = { code: "", name: "" };

export default function LodgeMarketSegment() {
  ...
  const query = useQuery({
    queryKey: [...QK, qs],
    queryFn: () => invoke("get_market_segments", { qs }),
  ...
  const createMut = useMutation({
    mutationFn: (d) => invoke("create_market_segment", {
      code: d.code ? parseInt(d.code) : null,
      name: d.name,
    }),
    onSuccess: () => { toast.success("Market segment created"); inv(); closeDialog(); },
    onError: (e) => toast.error(String(e)),
  });

  const updateMut = useMutation({
    mutationFn: (d) => invoke("update_market_segment", { id: d.id, name: d.name }),
    onSuccess: () => { toast.success("Market segment updated"); inv(); closeDialog(); },
    onError: (e) => toast.error(String(e)),
  });

  const toggleMut = useMutation({
    mutationFn: ({ id, is_active }) =>
      invoke("toggle_market_segment_active", { id, isActive: !is_active }),
    onSuccess: () => inv(),
    onError: (e) => toast.error(String(e)),
  });

  const deleteMut = useMutation({
    mutationFn: (id) => invoke("delete_market_segment", { id }),
    onSuccess: () => { toast.success("Market segment deleted"); inv(); setDeleteTarget(null); },
    onError: (e) => toast.error(String(e)),
  });
  ...
  async function openCreate() {
    ...
    try {
      const next = await invoke("get_next_master_code", { table: "market_segment" });
  ...
  const columns = useMemo(() => [
    ...
    {
      accessorKey: "name",
      header: ({ column }) => <DataTableColumnHeader column={column} title="Segment Name" />,
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-0.5">
          <Can perm="lodge-market-segment:update">
            <Tooltip>
          ...
          <Can perm="lodge-market-segment:delete">
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="ghost"
    ...
    <TooltipProvider>
        ...
        <Card>
          ...
          <CardContent>
            <DataTable
              ...
              emptyText="No market segments found."
              toolbar={
                <Can perm="lodge-market-segment:add">
      ...
      <Dialog open={dialog.open} onOpenChange={(o) => !o && closeDialog()}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>
              {dialog.mode === "create" ? "New Market Segment" : "Edit Market Segment"}
            </DialogTitle>
            <DialogDescription>
              {dialog.mode === "create"
                ? "Add a new lodge market segment."
                : "Update this market segment."}
            </DialogDescription>
          </DialogHeader>
