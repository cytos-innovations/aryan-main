...
export default function AccDebtor() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { toast.success("Debtor created"); inv(); closeDialog(); },
    onError: (e) => toast.error(String(e)),
  });

  const updateMut = useMutation({
    mutationFn: (d) =>
      invoke("update_debtor", {
        id: d.id,
        code: Number(d.code),
        name: d.name,
        address1: d.address1 || null,
    ...
    onSuccess: () => { toast.success("Debtor updated"); inv(); closeDialog(); },
    onError: (e) => toast.error(String(e)),
  });

  const toggleMut = useMutation({
    mutationFn: ({ id, is_active }) =>
      invoke("toggle_debtor_active", { id, isActive: is_active }),
    onSuccess: () => inv(),
    onError: (e) => toast.error(String(e)),
  });

  const deleteMut = useMutation({
    mutationFn: (id) => invoke("delete_debtor", { id }),
    onSuccess: () => { toast.success("Debtor deleted"); inv(); setDeleteTarget(null); },
    ...
    <div className="p-6 space-y-4">
      <Card>
        <CardHeader className="pb-3">
          <CardTitle>Debtor Ledger</CardTitle>
        </CardHeader>
        <CardContent>
          <DataTable
            columns={columns}
      ...
      <Dialog open={dialog.open} onOpenChange={(o) => !o && closeDialog()}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{isEditMode ? "Edit Debtor" : "Add Debtor"}</DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} onKeyDown={enterNav} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field>
                ...
                <SearchableSelect
                  ...
                  onSelect={(v) => setF("marketId", v || null)}
            ...
            <div className="border-t pt-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field>
                  ...
                  <SearchableSelect
                    options={[{ value: "C", label: "Credit (C)" }, { value: "D", label: "Debit (D)" }]}
                    value={form.openingCrdr}
                    onSelect={(v) => setF("openingCrdr", v)}
                    placeholder="Select…"
                  />
                </Field>
                <Field>
                ...
                <Field>
                  ...
                  <SearchableSelect
                    options={[{ value: "C", label: "Credit (C)" }, { value: "D", label: "Debit (D)" }]}
                    value={form.closingCrdr}
                    onSelect={(v) => setF("closingCrdr", v)}
                    placeholder="Select…"
                  />
                </Field>
              </div>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={isPending}>
                {isPending ? "Saving…" : isEditMode ? "Save Changes" : "Create Debtor"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
